...
class Post extends \app\Models\Model{
   ...
   public function author(){
        return \app\Users\UserFactory::fromID($this->wp_post->post_author);
   }
   ...
   public function image_post(){
      ...
      return \App\Posts\PostsFactory::fromId( $this->image_id() );
    ...
    private function get_taxonomy_objects( $tax_slug ){
         ...
         if(!empty($terms)){
            foreach($terms as $term) {
               $term_object = \App\TaxonomyTerms\TaxonomyTermFactory::fromObject($term);
    ...
    public function get_related_posts(){
        ...
        if(empty($post_ids)){
         ...
         if(count($post_ids)<6){
            ...
            $defaults = \App\RecommendedPosts\RecommendedPosts::get_global_defaults();
            shuffle($defaults);
            $post_ids = array_merge($post_ids,array_slice($defaults,0,6-count($post_ids)));
        }
        $posts = [];
        foreach($post_ids as $id){
            $post = \App\Posts\PostsFactory::fromID($id);
    ...
    function blankify($href){
        ...
        if(count($matches)){
            ...
            if(isset($parts['scheme'])&&isset($parts['host'])){
                preg_match("/((App\.com)|(breakthruradio\.com)|(b\-t\-r\.co))/",$url,$matches);
                if(count($matches)==0){
                    $href .= ' target="_blank"';
                }
            }
