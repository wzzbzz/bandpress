<?php

namespace bandpress\Views\ComponentViews;


class Nav{

    private $user;

    public function __construct($data=null){
        $this->user = new \bandpress\Models\User(wp_get_current_user());
    }

    public function __destruct(){

    }

    public function render(){
        // leverage wp api 
        is_user_logged_in() ? $this->render_user():$this->render_nouser();
    }

    private function render_user(){
      ?>
       <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
  <div class="container-fluid">
    <a class="navbar-brand" href="/users/<?= $this->user->slug();?>"><?= $this->user->display_name();?></a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarCollapse">
      <ul class="navbar-nav me-auto mb-2 mb-md-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="/wp-admin">Admin</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/pickenchicken">Picken' Chicken</a>
        </li>

        <li class="nav-item">
          <a class="nav-link" href="/actions/logout">logout</a>
        </li>


      </ul>
      <form class="d-flex">
        <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
        <button class="btn btn-outline-success" type="submit">Search</button>
      </form>
    </div>
  </div>
</nav>
<?php
    }

    private function render_nouser(){
      ?>
       <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
  <div class="container-fluid">
    <a class="navbar-brand" href="#">Fork The Internet</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarCollapse">
      <ul class="navbar-nav me-auto mb-2 mb-md-0">
        <li class="nav-item">
          <a class="nav-link" href="/register">Sign Up</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/login">Sign On</a>
        </li>
      </ul>
      
    </div>
  </div>
</nav>
<?php
    }
}
?>