...
class ApplicationController{
    ...
    */
    ...
    public function setPage( $wp ){
        ...
        if( is_search() ){
            $this->current_view = new \bandpress\Search\SearchResultsPage();
            return;
        }
        ...
        if(is_page()){            
            ...
            switch($pagename){
                case 'podcast-list':
                    $this->current_view = new \bandpress\Listen\PodcastListPage();
                    break;
                case 'podcast-archives':
                    $this->current_view = new \bandpress\Listen\PodcastListPage(false);
                    break;
                case 'playshow-json':
                    /* need to sanitize the inputs! */
                    $post = \bandpress\Posts\PostsFactory::fromSlug($_GET['postname']);
                    $this->current_view = new \bandpress\Listen\PodcastJSONView( $post );
                    break;
                case 'player':
                    $post =  \bandpress\Posts\PostsFactory::fromSlug( get_query_var( 'em_postname' ) ) ;
                    $this->current_view = new \bandpress\Listen\EmbeddedPlayerView( $post );
                    break;
                case 'radiotest':
                    $this->current_view = new \bandpress\Radio\RadioDataFeeds();
                    break;
                default:
        ...
        if (is_404()){
            ...
            $this->current_view = new \bandpress\ErrorPageView(404);
        }
