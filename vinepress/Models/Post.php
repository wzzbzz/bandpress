...
class Post extends Model{
   ...
   public function author(){
        return \vinepress\Users\UserFactory::fromID($this->wp_post->post_author);
   }
   ...
   public function image_post(){
      ...
      return \vinepress\Posts\PostsFactory::fromId( $this->image_id() );
    ...
    private function get_taxonomy_objects( $tax_slug ){
         ...
         if(!empty($terms)){
            foreach($terms as $term) {
               $term_object = \vinepress\TaxonomyTerms\TaxonomyTermFactory::fromObject($term);
    ...
    public function get_related_posts(){
        ...
        if(empty($post_ids)){
         ...
         if(count($post_ids)<6){
            ...
            $defaults = \vinepress\RecommendedPosts\RecommendedPosts::get_global_defaults();
            shuffle($defaults);
            $post_ids = array_merge($post_ids,array_slice($defaults,0,6-count($post_ids)));
        }
        $posts = [];
        foreach($post_ids as $id){
            $post = \vinepress\Posts\PostsFactory::fromID($id);
